window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function MarkStrip(props) {
			...
			// Dense packing: spacing is capped, so few bars stay close together
			// (cluster vertically centered); many bars shrink the spacing to fit.
			const spacing = Math.min(BAR_SPACING, geo.height / count);
			const clusterHeight = (count - 1) * spacing + HIT_SIZE;
			const clusterTop = geo.top + Math.max(0, (geo.height - clusterHeight) / 2);
			...
			const scrollTo = (index) => {
				...
				// Align the message with the top of the viewport; when there is
				// not enough content below it, clamp so it sits as high as
				// possible while the content below stays fully visible.
				const target = Math.min(
					Math.max(0, port.scrollTop + (rowRect.top - portRect.top) - TOP_MARGIN),
					maxScroll
				);
				activeRef.current = index;
				setActive(index);
			...
			for (let i = 0; i < count; i++) {
				const top = clusterTop + i * spacing - geo.top;
