window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *   the composer seat is `[data-composer-seat]`. The strip is a
		 *   viewport-fixed gutter (`position: fixed`, re-measured on resize /
		 *   scroll via ResizeObserver + rAF-throttled scroll listener).
		 * - Bars are distributed evenly along the gutter height (like a
		 *   scrollbar track), matching the desktop-app look.
		 * - Clicking scrolls with `scrollport.scrollTo({ behavior: 'smooth' })`
		 *   using the same scrollport-relative math the app itself uses
		 *   (`row.getBoundingClientRect().top - port.getBoundingClientRect().top`).
		...
		const STATIC_CSS =
			...
			".tm-bar{position:absolute;left:0;width:4px;height:" + BAR_HEIGHT + "px;border-radius:3px;background:var(--dsw-alias-label-tertiary,rgba(128,128,128,.5));cursor:pointer;pointer-events:auto;transition:background .15s ease,box-shadow .15s ease}" +
			".tm-bar:hover{background:var(--dsw-alias-label-secondary,rgba(180,180,180,.85))}" +
			".tm-bar.active{background:var(--dsw-alias-label-primary,#fff);box-shadow:0 0 6px var(--dsw-alias-label-primary,rgba(255,255,255,.6))}" +
		...
		function MarkStrip(props) {
			...
			// The white bar follows manual scrolling: it tracks the user message
			// whose row is nearest the top of the viewport (rAF-throttled).
			react.useEffect(() => {
				const port = document.querySelector(PORT_SELECTOR);
				if (!port) return;
				let raf = 0;
			...
			if (users.length === 0 || geo === null) return null;

			const count = users.length;
			const slot = geo.height / count;
			...
			const scrollTo = (index) => {
				...
				activeRef.current = index;
				setActive(index);
				port.scrollTo({ top: port.scrollTop + (rowRect.top - portRect.top) - 12, behavior: "smooth" });
			};

			const bars = [];
			for (let i = 0; i < count; i++) {
				const top = geo.top + slot * (i + 0.5) - BAR_HEIGHT / 2;
			...
			const hoveredTop = geo.top + slot * (hover + 0.5) - BAR_HEIGHT / 2;
			const tipTop = Math.max(geo.top, Math.min(hoveredTop + BAR_HEIGHT / 2 - 30, geo.top + geo.height - 90));
