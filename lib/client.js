window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * never make the tooltip read `users[hover]` out of range.
		 */
		...
		function MarkStrip(props) {
			...
			react.useEffect(() => {
				...
				const onScroll = () => {
					...
					raf = requestAnimationFrame(() => {
						...
						let index = activeIndexOf(rows.length, (i) => rows[i].getBoundingClientRect().top, limit);
			...
			react.useEffect(() => {
				setHover((h) => clampIndex(h, users.length));
				setActive((a) => clampIndex(a, users.length));
				if (activeRef.current >= users.length) activeRef.current = -1;
			}, [users.length]);
			...
			return react.createElement(react.Fragment, null,
					...
					hoverSafe >= 0 && react.createElement("div", {
						...
						react.createElement("div", { className: "tm-tip-head" },
							"#" + (hoverSafe + 1) + (fmtTimeOf(users[hoverSafe].time) ? " · " + fmtTimeOf(users[hoverSafe].time) : "")),
						react.createElement("div", { className: "tm-tip-body" }, previewOf(users[hoverSafe].content))
					)
			);
		}
